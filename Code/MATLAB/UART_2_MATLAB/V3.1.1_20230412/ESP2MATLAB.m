            ...
            for i=1:length(system.devices.arm.info.com)
                ...
                for j=1:system.devices.no
                    if contains(system.devices.history{j},system.devices.arm.info.com(i))
                        system.devices.history=[system.devices.history;strcat(["Arm(",num2str(i),"): "],system.devices.arm.info.com(i))];
                    end
                end
            end
        end
                ...
                for i=1:length(system.devices.state.available_port)
                    ...
                    for j=1:system.devices.no
                        if contains(system.devices.info{j,1},system.devices.state.available_port{i})
                            system.devices.history=[system.devices.history;strcat(["Arm(",num2str(i),"): "],string(system.devices.info{j,1}))];
                        end
                    end
                end
            end
