...
addpath('.\Results'); addpath('.\DarkTech'); addpath('.\Funcs');
...
if isempty(system.devices.info)
    ...
    % 2023.04.28
    % Temporarily the controller is designed to be in MATLAB code rather in
    % Arduino board to avoid the complex matrix computation with C
    % language. In the future we need to transfer it into Arduino
    % gradually.
    system.controller=0; % 0=off, 1=on.
...
if isfield(system,'base')
    ...
    if isfield(system,'human')
        system.human.temp.t0=tic; % To record the communication start time.
        system.human.temp.ti=0;
...
%     save(['Results/','system_',system.date,'.mat'],'system');
    ...
    % 2023.06.05
    % Plot the data for human angles using specific functions just for the
    % compactness of code.

    if ~isempty(system.human.record)
        data_plot(system.human.record,'human');
    end
